...
| `tools/push-to-github.mjs` | **浅克隆下会断历史** —— 本地最早那个提交的父提交没被拉下来，脚本把它映射成了空，复刻到 GitHub 上就成了**没有父提交的根提交** | 推上去后 `main` 只剩 2 个提交，v1.0 和「添加推送脚本」这两段历史**和分支脱开了**（对象还在库里，只是不在历史线上） | 新增 `resolveParents()`：父提交不在本次窗口内时，先确认它在远程存在，再**原样沿用它的哈希** |
