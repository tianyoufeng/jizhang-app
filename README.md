...
**当前版本：v1.1** —— 在 v1.0 基础上做了一轮界面与功能优化，并加入 iPhone 支持（详见第七节）。
...
1. 把 `记账本-v1.1.apk` 下载到手机（或者用数据线从电脑拷过去）
2. 在手机上点开这个文件
3. 手机会提示「不允许安装未知来源的应用」。去 **设置 → 安全 → 安装未知应用**，允许你刚才用的那个程序（文件管理器 / 浏览器 / 微信）安装，然后再点一次
4. 装好后桌面上会出现「记账本」图标

> ⚠️ 根目录里那个 `记账本-v1.0.apk` 是**优化之前**的旧版，别装错了。
> 现在用的是 `记账本-v1.1.apk`（`versionName=1.1`、`versionCode=2`）。
>
> 另外：这一版的签名密钥是**新生成**的（原因见第五节），所以如果手机上已经装了 v1.0，
> 直接装会报签名冲突 —— 先「设置 → 导出完整备份」，卸载旧版，装 v1.1 之后再恢复。
...
1. 取得网页文件 —— **仓库里已经打好了，直接解压 `记账本-v1.1-pwa.zip` 就是**（里面就是整个站点，`index.html` 在最外层）。
   也可以用源码自己构建：
   ```bash
   npm install
...
> **v1.1 的情况**：签 v1.0 的那把钥匙没有随仓库带过来（本来就不该带），
> 所以 v1.1 是**新生成**的一把，和 v1.0 不是同一把。这意味着：
>
> - 手机上**没装过** v1.0 → 直接装 v1.1，没问题
> - 手机上**已经装了** v1.0 → 装 v1.1 会报签名冲突，必须先卸载。
>   卸载会清掉 App 数据，**先「设置 → 导出完整备份」再卸**，装完 v1.1 恢复回来即可。
>
> 从 v1.1 往后都用同一把钥匙，就不会再有这个问题了。
...
├── 记账本-v1.1.apk         安卓安装包
├── 记账本-v1.1-pwa.zip     iPhone 用的 PWA 包（解压后自托管，Safari 添加到主屏幕）
├── src/
...
## 七、这次优化都改了什么（v1.0 → v1.1）

改动集中在「手机上的观感」和「真正会用到但原来没有的功能」，**没有引入任何新框架、新依赖、新网络请求**，仍然是一个纯本地 App。逐条对照 `记账本-优化建议.md` 里的清单：
