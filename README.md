...
**当前版本：v1.4** —— **账单和统计都能按「周 / 月 / 年」查了**，另外顺手把两个「看着别扭」的地方改了：红包换成 🧧 红图标、收入红 / 支出绿（改回中文习惯）、主题切换按钮加了一圈底色高亮。上一版 v1.3 把 App 图标换成 **￥**（金 → 橙渐变），v1.2 加了**分类预算**。四版都在第七节。
...
1. 把 `记账本-v1.4.apk` 下载到手机（或者用数据线从电脑拷过去）
2. 在手机上点开这个文件
3. 手机会提示「不允许安装未知来源的应用」。去 **设置 → 安全 → 安装未知应用**，允许你刚才用的那个程序（文件管理器 / 浏览器 / 微信）安装，然后再点一次
4. 装好后桌面上会出现「记账本」图标

> ⚠️ 根目录里那个 `记账本-v1.0.apk` 是**优化之前**的旧版，别装错了。
> 现在用的是 `记账本-v1.4.apk`（`versionName=1.4`、`versionCode=5`）。
>
> **升级路径**：v1.1 → v1.2 → v1.3 → v1.4 用的都是**同一把签名密钥**，直接把新包装上去就行，
> 不用卸载，数据也不会动。
>
> 只有**从 v1.0 往上装**（不管是装 v1.1、v1.2、v1.3 还是 v1.4）才会报签名冲突 ——
...
1. 取得网页文件 —— **仓库里已经打好了，直接解压 `记账本-v1.4-pwa.zip` 就是**（里面就是整个站点，`index.html` 在最外层）。
   也可以用源码自己构建：
   ```bash
   npm install
...
- 上方同样是「**按周 / 按月 / 按年**」三格，切换逻辑和账单页一模一样（含「再点一次回到当下」）
...
改完代码后跑这个，它会用真浏览器把 **108 项**功能自动点一遍 —— 记账、编辑、删除、筛选、搜索、导出、备份恢复、多账本、总预算、分类预算（含圆点的三种颜色）、主题、周/月/年三种粒度（含翻页边界和「切粒度不丢位置」）、刷新后数据是否还在，全部核对数字：
...
>
> - 手机上**没装过**这个 App → 直接装 v1.4，没问题
> - 手机上**装的还是 v1.0** → 装 v1.4 会报签名冲突，必须先卸载。
>   卸载会清掉 App 数据，**先「设置 → 导出完整备份」再卸**，装完恢复回来即可。
> - 手机上**装的是 v1.1 / v1.2 / v1.3** → 直接装 v1.4 就行，**同一把钥匙**，不用卸载、数据不动。
>
...
├── 记账本-v1.4.apk         安卓安装包
├── 记账本-v1.4-pwa.zip     iPhone 用的 PWA 包（解压后自托管，Safari 添加到主屏幕）
├── src/
