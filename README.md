...
**当前版本：v1.6** —— 账单和统计的查询时间**可以自己圈了**：在那排「周 / 月 / 年」旁边加了「**自定义**」，点开选两个日期，或者用「近 7 天 / 近 30 天 / 近 3 个月 / 今年 / 去年」一键；统计页还会自己决定柱子按天还是按周分。上一版 v1.5 把那排按钮的字缩成了「**周 / 月 / 年**」并修掉统计页右侧的死区；v1.4 让**账单和统计都能按周 / 月 / 年查**，并把红包换成 🧧 红图标、收入红 / 支出绿（改回中文习惯）。v1.3 换的 App 图标是 **￥**（金 → 橙渐变），v1.2 加了**分类预算**。六版都在第七节。
...
![自定义时间](docs/screenshots/6-range.png)
...
1. 把 `记账本-v1.6.apk` 下载到手机（或者用数据线从电脑拷过去）
2. 在手机上点开这个文件
3. 手机会提示「不允许安装未知来源的应用」。去 **设置 → 安全 → 安装未知应用**，允许你刚才用的那个程序（文件管理器 / 浏览器 / 微信）安装，然后再点一次
4. 装好后桌面上会出现「记账本」图标

> ⚠️ 根目录里那个 `记账本-v1.0.apk` 是**优化之前**的旧版，别装错了。
> 现在用的是 `记账本-v1.6.apk`（`versionName=1.6`、`versionCode=7`）。
>
> **升级路径**：v1.1 → v1.2 → v1.3 → v1.4 → v1.5 → v1.6 用的都是**同一把签名密钥**，
> 直接把新包装上去就行，不用卸载，数据也不会动。
>
> 只有**从 v1.0 往上装**（不管是装 v1.1、v1.2、v1.3、v1.4、v1.5 还是 v1.6）才会报签名冲突 ——
...
1. 取得网页文件 —— **仓库里已经打好了，直接解压 `记账本-v1.6-pwa.zip` 就是**（里面就是整个站点，`index.html` 在最外层）。
   也可以用源码自己构建：
   ```bash
   npm install
...
**顶栏**（每个页面都在）：左边是当前账本名，点一下切换账本；中间是页面标题；
右边那个圆按钮是**深浅色切换** —— 带一圈暖色底，太阳和月亮哪个亮着，就说明当前是哪种主题。

### 记账

填金额 → 点分类 → 保存。三步就能记一笔。

- 上面切「支出 / 收入」，**收入用红色、支出用绿色**（按中文习惯，涨红跌绿那一套）
...
顶部五格切「**周 / 月 / 年 / 全部时间 / 自定义**」，默认按月看本月。

- 左右箭头翻周期，翻到头会自动置灰（左边到头＝最早那笔账所在的周期，右边到头＝当下）
- 标题跟着粒度变：按周是「9月14日–20日」，按月是「9月」，按年是「2026年」
- 切粒度**不会丢位置**：翻到 3 月再点「按年」，会落到 2026 年而不是蹦回今天。
  而且如果你正在看的那一段**包含今天**，切过去就落在今天所在的新周期上
  ——（在当月点「按周」落到本周，而不是 9 月 1 日那一周）
- 已经切到一个粒度上了再点一次，等于**回到当下** —— 翻了好几页想跳回来，不用一格一格按回去
- **「自定义」＝ 自己圈一段时间**。点开后选开始和结束两个日期，或者直接用快捷档
  「近 7 天 / 近 30 天 / 近 3 个月 / 今年 / 去年」。选完顶部变成一条区间条
  （`9月13日–19日` ＋ `共 7 天 · 点这里改时间`），**整条都能点**，点一下重新选。
  面板里先垫着当前这个月（从「月」点进来时）、上次选过的那一段，改两下就能用；
  点「取消」不会动原来那一段
- 顶部是这一段的收入、支出、结余
- 下面按天分组，同一天的归在一起，组标题显示当天的收支小计（切到年看时，非今年的日期会自动带上年份）
- 「全部时间」是**跨周期**搜旧账用的：找「三个月前买空调那笔」，不用一个周期一个周期翻
- **点任意一条记录**，可以：修改、再记一笔一样的（复制到今天）、删除
- **删除可以撤销** —— 删完底部会弹一条提示，6 秒内点「撤销」就回来了；过了就真删了
- 支持按分类筛选、搜备注或分类名。筛完之后顶部的合计会跟着筛选结果变，不会让你看错
- 从底部栏重新点「账单」，筛选和翻页位置会清掉，回到当月全貌（粒度保持你上次选的）

### 统计

- 上方是「**周 / 月 / 年 / 自定义**」四格，切换逻辑和账单页一模一样（含「再点一次回到当下」）。
  统计页没有「全部时间」—— 饼图和趋势得有个明确的时间段才有意义
- **自定义时间段**：和账单页共用同一个选择面板。
  选定后**柱状图会自己决定按天还是按周分**：一个月内按天、半年内按周、四年内按月，再长按年
  （柱子最多 8 根 —— 30 天按天切就是 30 根，横轴标签会叠成一团）。
  图表标题会写明是按什么分的，比如「区间趋势 · 按周」
- **本段总结**：这一个周期（或这一段自定义时间）的收入、支出、结余
- **环比**：总结下面一行告诉你「支出比上月多花 412.30 元（+18%）」，**多花是红、少花是绿**；
  粒度换成周 / 年，这句话会自动变成「比上周 / 比上年」；
  自定义时间段没有「上月」这个说法，就跟**紧挨着它之前、长度相同的那一段**比（写「上一段」）
- **支出构成 / 收入构成**：分类占比圆环图，下面列出每个分类的金额和百分比
- **点图例上的某个分类**，会直接跳到账单页看这个分类的明细 —— 并且带着当前粒度过去，
  所以「按年看到的餐饮」点进去也是**整年**的餐饮，不会是某一个月；
  在自定义时间段里点，账单页也还是那一段
- **最近 6 周 / 6 个月 / 6 年趋势**：柱状图，**红的是收入、绿的是支出**（和列表里的数字一个色）
- 上方的箭头可以翻到以前看（自定义时间段下没有箭头 —— 区间是自己定的，没有「上一段」）
...
改完代码后跑这个，它会用真浏览器把 **137 项**功能自动点一遍 —— 记账、编辑、删除、筛选、搜索、导出、备份恢复、多账本、总预算、分类预算（含圆点的三种颜色）、主题、周/月/年三种粒度（含翻页边界、切粒度不丢位置、按钮文案和铺满整行）、**自定义时间段**（面板、快捷档、日期校验、空区间、统计页的分桶与环比、从统计跳账单时区间要跟着走），刷新后数据是否还在，全部核对数字：
...
>
> - 手机上**没装过**这个 App → 直接装 v1.6，没问题
> - 手机上**装的还是 v1.0** → 装 v1.6 会报签名冲突，必须先卸载。
>   卸载会清掉 App 数据，**先「设置 → 导出完整备份」再卸**，装完恢复回来即可。
> - 手机上**装的是 v1.1 / v1.2 / v1.3 / v1.4 / v1.5** → 直接装 v1.6 就行，**同一把钥匙**，不用卸载、数据不动。
>
...
├── 记账本-v1.6.apk         安卓安装包
├── 记账本-v1.6-pwa.zip     iPhone 用的 PWA 包（解压后自托管，Safari 添加到主屏幕）
├── src/
│   ├── main.js             入口：启动、主题、Tab 切换、顶部账本切换
│   ├── db.js               IndexedDB 封装（增删改查、批量、清空）
│   ├── store.js            全部业务逻辑：账本、分类、记录、统计、导入导出
│   ├── utils.js            金额换算、日期、周期（周/月/年）、自定义区间、格式化、图表配色
│   ├── ui.js               弹层、确认框、轻提示、选项卡
│   ├── rangePicker.js      自定义时间段的选段面板（账单页、统计页共用）
...
之前账单和统计只能按「周 / 月 / 年」这三种固定粒度看。想算「8月20日到9月5日」
或者「最近 3 个月」花了多少，只能一个周期一个周期地翻。

- **账单页和统计页都多了一格「自定义」**。点开是一个选段面板：
  两个日期框 ＋「近 7 天 / 近 30 天 / 近 3 个月 / 今年 / 去年」五个快捷档。
  面板会先垫上你当前正在看的那个月，改两下就能用；上次选过的那一段也会记住；
  点「取消」什么都不动。
- **选完顶部换成一条区间条**：`9月13日–19日` ＋ `共 7 天 · 点这里改时间`。
  整条都能点，点一下重新打开面板改。
  这条上没有 `‹ ›` 箭头 —— 区间是自己定的，没有「上一段 / 下一段」，
  留着两个点了没用的箭头只会让人误点。
- **日期填反了会直接说**：开始晚于结束时面板里会写「开始日期不能晚于结束日期」
  并把「看这段」按钮置灰，**不会偷偷帮你换过来** ——
  换掉的话你会以为自己点错了，却看不出错在哪。结束日期选到未来的会被压回今天。
- **统计页的柱状图会自己挑粒度**：一个月内按天、半年内按周、四年内按月，再长按年；
  柱子最多 8 根（30 天按天切就是 30 根，横轴标签会叠成一团）。
  图表标题会写明是按什么分的，比如「区间趋势 · 按周」。
- **环比多了一个说法**：自定义时间段没有「上月」，就跟**紧挨着它之前、长度相同的那一段**比，
  文案写「支出比上一段多花 89.50 元（+45%）」。
- **从统计点图例跳到账单，看的是同一段**：在自定义时间段里点「餐饮」，
  跳过去仍是这一段里的餐饮，不会被甩回本月。
- **分类预算和「本月预算」没跟着变，仍然按自然月算** ——
  预算是按月设的，「这个月的预算」在你按周看的时候也该是这个月的，不该变成那一周的。
- 数据和备份格式一行没动，v1.5 的备份可以直接恢复进来。

### v1.5 —— 粒度按钮改成一个字

就一件事，加一个顺手修的：

- **按钮文案**：账单和统计顶部那排从「按周 / 按月 / 按年」缩成「**周 / 月 / 年**」。
  四个格子横排在一起，一个字一眼能看全；「按周」那种写法反而把格子撑长，
  和旁边的「全部时间」挤在一起。文案只有一处定义（`utils.js` 的 `PERIOD_BTN_TEXT`），
  账单和统计都从它取，改一处两边同时生效。
  > 标题里的「9月14日–20日」和环比里的「比上周多花」**没跟着改** ——
  > 那两处是句子里的词，不是按钮。
- **顺手修掉统计页那排按钮右边空出一格**：分栏原来写死四列，
  账单页正好四个按钮看不出问题，统计页只有三个，右边就空出整整一格
  （量出来容器 362px、最后一个按钮右边缘只到 269，**空了 93px**）。
  改成「列数跟着按钮数走」：账单页仍是 4 等分，统计页变成 3 等分铺满整行。
  这是 v1.4 引入这排按钮时就带着的问题，当时只看了账单页。
- 数据和逻辑一行没动，v1.4 的备份可以直接恢复进来。

### v1.4 —— 周 / 月 / 年 + 配色改回中文习惯

这一版是三件事：**账单和统计都能按周 / 月 / 年查了**，另外把两处「看着别扭」的地方改掉。
数据结构和备份格式**没变**，v1.3 的备份可以直接恢复进来。

#### 按周 / 看年

- **账单页**：顶部从「本月 / 全部时间」两格，变成「按周 / 按月 / 按年 / 全部时间」四格。
- **统计页**：顶部加了三格粒度切换，整套卡片跟着走 —— 总结卡标题、环比文案
  （比上周 / 上月 / 上年）、趋势图（最近 6 周 / 6 个月 / 6 年）、坐标轴短标签。
- **翻周期不丢位置**：正在看的那一段**包含今天**时，切粒度就落到今天所在的新周期；
  看的是过去某一段（比如 3 月）时，才用它第一天换算。
  这条是踩过的坑 —— 起先一律按「第一天」算，结果在 9 月点「按周」落到了 8/31–9/6 那一周，一条记录都没有。
- **再点一次当前粒度 = 回到当下**，和底栏「再点一次当前 tab 回到顶部」一个路子。
- **点统计的图例跳账单**时会把粒度一起带过去，所以「按年看到的餐饮」点进去是**整年**的。
- 一周从**周一**开始。
- **分类预算仍然只按自然月算** —— 预算本来就是「每月多少」，切到按周看不该跟着变。

#### 配色改回中文习惯

- **红＝收入、绿＝支出**（原来是反的）。国内看惯了股市那套涨红跌绿，支出标红反而像在报警。
- 「收入 / 支出」这两个小标题也一起上色了，原来一律灰字，得回头对一眼才知道哪个是哪个。
- 顺手抽了个新变量 `--safe`：预算进度条的「正常态」原来借的就是那个绿，对调后会被一起染红。
  它说的是「状态正常」，跟「支出」只是碰巧同色，所以拆开。
- 「比上月多花」那句走 `--danger`（说「花多了、不好」），和收支配色是两码事，也拆开了。

#### 红包图标 + 主题按钮

- **「红包」分类的图标 💵 → 🧧**。分类存在手机本地，光改默认值对已装旧版的人没用，
  所以补了一次**数据迁移**：只认「名字还叫红包、图标还是 💵」这一种，你自己挑过图标的不动。
  ⚠️ 🧧 要 Emoji 11.0（Android 9+）才画得出来 —— 这是全项目唯一一个超出 Unicode 9.0 的图标，
  实在没有更贴切的替代品，代码里已注明。
- **主题切换按钮做显眼**：38px 暖金圆底 + 加粗图标，深浅色各一套色值，深色下太阳还带一层外发光。
  原来是一枚 19px 灰线图标直接压在白顶栏上，基本看不见。
