...
**当前版本：v1.5** —— 把账单和统计顶部那排粒度按钮的字缩成了「**周 / 月 / 年**」（原来写「按周 / 按月 / 按年」，把格子撑长了），顺手修掉统计页那排按钮右边空出一格的问题。上一版 v1.4 让**账单和统计都能按周 / 月 / 年查**，并把红包换成 🧧 红图标、收入红 / 支出绿（改回中文习惯）、主题按钮加了一圈底色高亮。v1.3 换的 App 图标是 **￥**（金 → 橙渐变），v1.2 加了**分类预算**。五版都在第七节。
...
1. 把 `记账本-v1.5.apk` 下载到手机（或者用数据线从电脑拷过去）
2. 在手机上点开这个文件
3. 手机会提示「不允许安装未知来源的应用」。去 **设置 → 安全 → 安装未知应用**，允许你刚才用的那个程序（文件管理器 / 浏览器 / 微信）安装，然后再点一次
4. 装好后桌面上会出现「记账本」图标

> ⚠️ 根目录里那个 `记账本-v1.0.apk` 是**优化之前**的旧版，别装错了。
> 现在用的是 `记账本-v1.5.apk`（`versionName=1.5`、`versionCode=6`）。
>
> **升级路径**：v1.1 → v1.2 → v1.3 → v1.4 → v1.5 用的都是**同一把签名密钥**，
> 直接把新包装上去就行，不用卸载，数据也不会动。
>
> 只有**从 v1.0 往上装**（不管是装 v1.1、v1.2、v1.3、v1.4 还是 v1.5）才会报签名冲突 ——
...
1. 取得网页文件 —— **仓库里已经打好了，直接解压 `记账本-v1.5-pwa.zip` 就是**（里面就是整个站点，`index.html` 在最外层）。
   也可以用源码自己构建：
   ```bash
   npm install
...
- 上方同样是「**周 / 月 / 年**」三格，切换逻辑和账单页一模一样（含「再点一次回到当下」）
- **本段总结**：这一个周期（或这一年）的收入、支出、结余
...
改完代码后跑这个，它会用真浏览器把 **111 项**功能自动点一遍 —— 记账、编辑、删除、筛选、搜索、导出、备份恢复、多账本、总预算、分类预算（含圆点的三种颜色）、主题、周/月/年三种粒度（含翻页边界、切粒度不丢位置、按钮文案和铺满整行）、刷新后数据是否还在，全部核对数字：
...
>
> - 手机上**没装过**这个 App → 直接装 v1.5，没问题
> - 手机上**装的还是 v1.0** → 装 v1.5 会报签名冲突，必须先卸载。
>   卸载会清掉 App 数据，**先「设置 → 导出完整备份」再卸**，装完恢复回来即可。
> - 手机上**装的是 v1.1 / v1.2 / v1.3 / v1.4** → 直接装 v1.5 就行，**同一把钥匙**，不用卸载、数据不动。
>
